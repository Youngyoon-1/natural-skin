...
<body>
	...
	<form action="order2Do" method="post">
	<table class="order">
		<tr>
			<th>주문상품[${fn:length(list)}건]</th>	
			<th>상품명</th>
			<th>수량</th>
			<th>가격</th>
		</tr>
		...
		<c:forEach var="basket" items="${list}">
			...
			<input name="product_id" value="${basket.product_id}" type="hidden">
			<input name="order_detail_amount" value="${basket.product_amount}" type="hidden">
			<input name="order_detail_price" value="${basket.basket_price}" type="hidden">
			<input name="product_name" value="${basket.product_name}" type="hidden">
			<input name="product_price" value="${basket.product_price}" type="hidden">
			<input name="product_img_path" value="${basket.product_img_path}" type="hidden">			
			<tr>
				<td><img class="orderImg" src="upload/${imgPath[0]}"></td>
				<td>${basket.product_name}</td>
				<td>${basket.product_amount}개</td>
				<td>
				<c:set var="price" value="${price + basket.basket_price}" />
				<fmt:formatNumber value="${basket.basket_price}" pattern="#,###" />원</td>
			</tr>
		</c:forEach>
		<tr>
			<td></td>
			...
			<td>총상품금액</td>
			<td>
				<fmt:formatNumber value="${price}" pattern="#,###" />원</td>
		</tr>
	</table>
	<h3 style="margin-left: 7.5%;">배송정보</h3>
		...
		<input name="member_order_price" value="${price+2500}" type="hidden">
		<div class="main">
			<div class="recipient">
		...
		<table class="payment">
			...
			<tr>
				<th>총 결제 금액</th>
				<td>
					<fmt:formatNumber value="${price+2500}" pattern="#,###" />원
				</td>
			</tr>
		</table>
		<div id="btns">
